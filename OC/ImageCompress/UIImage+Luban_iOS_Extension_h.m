...
+ (NSData *)lubanCompressImage:(UIImage *)image withMask:(NSString *)maskName {
    ...
    float imageCapacity = imageData.length / 1024.0;
    if (imageCapacity < 250) {
        return UIImageJPEGRepresentation(image, 0.6);
    } else if (imageCapacity < (1 * 1024)) {
        return UIImageJPEGRepresentation(image, 0.5);
    } else if (imageCapacity < (2 * 1024)) {
    ...
    double scale = (shortSide * 1.0) / (longSide * 1.0);

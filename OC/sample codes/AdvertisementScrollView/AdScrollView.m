...
#define HIGHT self.bounds.origin.y //由于_pageControl是添加进父视图的,所以实际位置要参考滚动视图的y坐标
...
@interface AdScrollView ()
{
    ...
    //用于确定滚动是由人导致的还是计时器到了,系统帮我们滚动的,YES,则为系统滚动,NO则为客户滚动(ps.
    //在客户端中客户滚动一个广告后,这个广告的计时器要归0并重新计时)
    BOOL _isTimeUp;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _moveTime = [NSTimer scheduledTimerWithTimeInterval:chageImageTime
                                                     target:self
...
- (void)setPageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle
{
    ...
    //根据 pageControl 的显示样式调整 pageControl 的位置
...
//由于PageControl控件必须要添加在滚动视图的父视图上(添加在滚动视图上的话会随着图片滚动,而达不到效果)
- (void)addPageControl
{
